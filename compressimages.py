...
# C:\Users\Vitor Fortunato\Documents\Vitor\spatial_storage\ger_levels\blocks_tiles\blocks\8\131
# python "C:\Users\Vitor Fortunato\Documents\Vitor\prj\py_compress_images\compressimages.py" --mode compress "C:\Users\Vitor Fortunato\Documents\Vitor\spatial_storage\ger_levels\blocks_tiles\blocks"
...
class ProcessBase:
    logging.basicConfig(filename=r'C:\Users\Vitor Fortunato\Documents\Vitor\prj\py_compress_images\log.txt',
                        level=logging.INFO)
        ...
        try:
            ...
            with open(filename, 'rb') as file:
                ...
                if format != 'PNG' and format != 'JPEG':
                    print('Ignoring file "' + str(filename) + '" with unsupported format ' + format)
                    return False
            ...
            stderr.write('Failure whilst processing "' + str(filename) + '": ' + str(e) + '\n')
            logger.info('error on file: ' + str(filename) + '": ' + str(e))
        finally:
            if not ok:
                logger.info('error on file: ' + str(filename))
...
class DeleteBackupImage(ProcessBase):
    ...
    def processfile(self, filename):
        ...
        try:
            ...
            stderr.write('Failed to delete backup file "' + str(filename) + '": ' + str(e) + '\n')
            return False
